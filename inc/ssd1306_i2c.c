...
void ssd1306_update_display(uint8_t *buffer) {
    // Configurar os endereços de coluna e página no display
    uint8_t commands[] = {
        ssd1306_set_column_address, 0x00, ssd1306_width - 1,  // Colunas: início e fim
        ssd1306_set_page_address, 0x00, (ssd1306_height / 8) - 1 // Páginas: início e fim
    };

    // Enviar os comandos ao display
    ssd1306_send_command_list(commands, sizeof(commands) / sizeof(commands[0]));

    // Enviar os dados do buffer ao display
    ssd1306_send_buffer(buffer, ssd1306_width * (ssd1306_height / 8));
}

void ssd1306_clear(void) {
    uint8_t buffer[1024] = {0}; // Buffer para um display de 128x64
    ssd1306_update_display(buffer); // Substitua pela função correta para atualizar o display
}